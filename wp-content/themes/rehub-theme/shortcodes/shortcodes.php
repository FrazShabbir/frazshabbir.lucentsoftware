...
if (is_user_logged_in()) {
	...
	if(function_exists('mycred_render_shortcode_my_balance') && function_exists('mycred_render_shortcode_my_balance')){
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if( $term->parent == '0' ){
					...
					$term_titles .= '<div '. $right_style .'><h5 class="rh-heading-hover-color blockstyle mb10 fontnormal font110 mt0"><a class="" href="'. esc_url(get_term_link($term)) .'" title="'. esc_attr(sprintf( esc_html__('View all post filed under %s', 'rehub-theme' ), $term->name)) .'">'. esc_html($term->name) . '</a></h5>';
					$term_titles .= '<div class="subcategortes font70 lineheight15 blackcolor">'. $term_childes .''. $ellipsis .'</div></div>'; 
					$term_titles .= ($thumbnail_url) ? '<a href="'. esc_url(get_term_link($term)) .'" title="'. esc_attr(sprintf( esc_html__('View all post filed under %s', 'rehub-theme' ), $term->name)) .'" class="rh-flex-right-align"><img src="'. esc_url($thumbnail_url) .'" alt="'. esc_attr($term->name) .'" width="'. $imageheight .'" /></a>' : '';					
					$term_titles .= '</div>';
				}
			}
			...
			<div class="csspopup" id="addfrontdeal_<?php echo ''.$rand;?>">
				<div class="csspopupinner addfrontdeal-popup">
					...
					<?php if ( in_array( $role, (array) $current_user->roles )):?>
						...
						<?php if (false === in_array($current_user->ID, $multioffer_names)):?>
							<div class="rehub-offer-popup">
								<div class="re_title_inmodal"><?php esc_html_e('Add an Offer', 'rehub-theme'); ?></div>
								<form id="rehub_add_offer_form_modal" action="<?php echo home_url( '/' ); ?>" method="post">
									<div class="re-form-group mb20">
										<label for="rehub_product_name"><?php esc_html_e('Name of product', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_name" id="rehub_product_name" type="text" />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_url"><?php esc_html_e('Offer url', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_url" id="rehub_product_url" type="url" required />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_price"><?php esc_html_e('Offer sale price (example, $55)', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_price" id="rehub_product_price" type="text" />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_desc"><?php esc_html_e('Short description', 'rehub-theme') ?><span></span></label>
						...
						<?php else:?>
							<?php esc_html_e('You already added your deal to this post', 'rehub-theme');?>
						<?php endif;?>
					<?php else:?>
					...
					<div class="rehub-offer-popup-ok font110 rhhidden">
						<div class="re_title_inmodal"><?php esc_html_e('Send Offer', 'rehub-theme'); ?></div>
							<?php printf( esc_html__('<strong>Thank you, %s!</strong> Your offer has been sent', 'rehub-theme'), $current_user->display_name ); ?>
					</div>
				</div>				
			</div>
		<?php endif;?>
